...
@NamedQueries(
        {
                ...
                @NamedQuery(name="findPolicyIdByTypeAndConfigKeyValue", query="select p.id from PolicyEntity p join p.config c where p.resourceServer.id = :serverId and p.type = :type and key(c) = :key and value(c) like concat('%',:value,'%')"),
